# A2500 MAIN HOME ANALYSIS Phasor { .screen-title-fit }

<div class="screen-link-wrap">
  <img src="/assets/screens/MAIN/HOME/MAIN_HOME_ANALYSIS_Phasor.png" alt="A2500 MAIN HOME ANALYSIS Phasor">
  ...
  <a class="screen-hotspot" href="../home-analysis-phasor-2/" title="Toggle Voltage phasor display" aria-label="Toggle Voltage phasor display" style="left:66.8%; top:26.4%; width:15.0%; height:8.9%;">Voltage</a>
  <a class="screen-hotspot" href="../home-analysis-phasor-3/" title="Toggle Current phasor display" aria-label="Toggle Current phasor display" style="left:82.0%; top:26.4%; width:15.0%; height:8.9%;">Current</a>
...
<details class="policy-details">
  ...
  <div class="policy-details__body">
    ...
    <details class="policy-details">
      <summary>기준 원 표시</summary>
      <div class="policy-details__body">
        <ul>
          <li>기준 원은 3등분 기준으로 표시한다.</li>
          <li>전압은 바깥쪽 1개 원, 전류는 2/3 지점, ZCT 전류는 1/3 지점에 표시한다.</li>
        </ul>
      </div>
    </details>
    <details class="policy-details">
      <summary>3상 모듈 전용 메뉴</summary>
      <div class="policy-details__body">
        <ul>
          <li><code>Curr. Symm.</code>와 <code>Curr. Unbla. %</code> 서브 메뉴는 3상 모듈에서만 표시한다.</li>
        </ul>
      </div>
    </details>
    <details class="policy-details">
      <summary>상설정에 따른 표시와 색상</summary>
      <div class="policy-details__body">
        <ul>
          <li><code>L-N</code> 설정(<code>AN</code>, <code>NA</code> 등): <code>A</code>, <code>B</code>, <code>C</code>로 표시한다.</li>
          <li><code>L-L</code> 설정(<code>AB</code>, <code>BA</code> 등): <code>AB</code>, <code>BC</code>, <code>CA</code>로 표시하며 역상도 표시한다.</li>
          <li>상별 선 색상은 <code>문서정보 &gt; 개발참고사항 &gt; Phasor 상별 선색</code>의 <code>IA</code>, <code>IB</code>, <code>IC</code> 기준을 따른다.</li>
          <li>선간 표시는 앞쪽 상의 색상을 사용한다.</li>
          <li><code>IA</code> 회색: <code>AN</code>, <code>NA</code>, <code>AB</code>, <code>AC</code></li>
          <li><code>IB</code> 분홍색: <code>BN</code>, <code>NB</code>, <code>BA</code>, <code>BC</code></li>
          <li><code>IC</code> 하늘색: <code>CN</code>, <code>NC</code>, <code>CA</code>, <code>CB</code></li>
        </ul>
      </div>
    </details>
    <h4>공통 정책</h4>
    ...
    <h4>적용 완료</h4>
    <details class="policy-details">
      <summary>적용 완료: 제목과 기준원</summary>
      <div class="policy-details__body">
        <p><code>Policy-23-0006</code>에 따라 데이터뷰 제목은 <code>Phasor [Vector Diagram]</code>이 아니라 <code>Phasor</code>로 표시한다. Phasor 그래프 기준원은 기존 <code>1</code>, <code>1/2</code>, <code>1/4</code> 방식이 아니라 3등분 기준원인 <code>1</code>, <code>2/3</code>, <code>1/3</code> 비율을 사용한다.</p>
        <p>근거: <code>Policy-23-0006</code></p>
      </div>
    </details>
    <details class="policy-details">
      <summary>적용 완료: VLN 기본 선택</summary>
      <div class="policy-details__body">
        <p><code>Policy-24-0024</code>는 전압 표시의 기본 선택을 일반적으로 <code>VLL</code>로 두되, <code>HOME &gt; ANALYSIS &gt; Phasor</code>만 예외적으로 <code>VLN</code>을 기본 선택으로 정한다. 현재 화면은 <code>VLN</code>이 선택되어 있으므로 정책과 맞다.</p>
        <p>근거: <code>Policy-24-0024</code></p>
      </div>
    </details>
    <details class="policy-details">
      <summary>적용 완료: 전압/전류 체크 버튼</summary>
      <div class="policy-details__body">
        <p><code>Policy-24-0025</code>는 Phasor의 <code>Voltage</code>/<code>Current</code> 체크 버튼 동작을 Harmonics, Waveform과 유사하게 정리한다. 체크를 해제할 때 그래프 안의 해당 벡터선만 숨기고, 우측 숫자값 영역은 유지한다.</p>
        <p>근거: <code>Policy-24-0025</code></p>
      </div>
    </details>
    <details class="policy-details">
      <summary>적용 완료: 색상과 표시값</summary>
      <div class="policy-details__body">
        <p><code>Policy-23-0012</code>는 전압, 전류, ZCT 색상 구분을 정의한다. 화면의 <code>A</code>, <code>B</code>, <code>C</code>, <code>ZCT</code> 라벨 색상은 각 범주를 구분하는 용도로 해석한다.</p>
        <p>근거: <code>Policy-23-0012</code></p>
      </div>
    </details>
  </div>
</details>
